...
export default function MessagesScreen() {
    ...
    try {
      ...
      if (sentResponse.success && receivedResponse.success) {
        ...
        allMessages.forEach((message) => {
          ...
          const otherUserEmail = message.sender_id === user.id ? message.receiver_email : message.sender_email;
  ...
  const handleSendMessage = async () => {
    try {
      if (!user?.id || !newMessage.receiver_email.trim() || !newMessage.content.trim()) {
  ...
  const handleReplyMessage = async (content: string, receiverId: string, receiverEmail: string) => {
    try {
  ...
  const markAsRead = async (messageId: string) => {
    try {
      await messagesService.update(messageId, { is_read: true });
    } catch (error) {
  ...
  const ConversationItem = ({ conversation }: { conversation: Conversation }) => (
    ...
    >
      <View style={styles.conversationHeader}>
        <ThemedText style={styles.conversationEmail}>{conversation.otherUserEmail}</ThemedText>
  ...
  const MessageItem = ({ message }: { message: Message }) => {
    ...
    return (
      <View style={[styles.messageItem, isOwn ? styles.ownMessage : styles.otherMessage]}>
        <ThemedText style={[styles.messageContent, isOwn ? styles.ownMessageContent : styles.otherMessageContent]}>
          {message.content}
        </ThemedText>
        <ThemedText style={[styles.messageTime, isOwn ? styles.ownMessageTime : styles.otherMessageTime]}>
    ...
    return (
      <View style={styles.container}>
        <View style={styles.chatHeader}>
          ...
          <ThemedText style={styles.chatTitle}>{selectedConversation.otherUserEmail}</ThemedText>
          <View style={{ width: 24 }} />
        </View>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              if (newMessage.content.trim()) {
                handleReplyMessage(newMessage.content, selectedConversation.otherUserId, selectedConversation.otherUserEmail);
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <ThemedView style={styles.emptyState}>
            ...
            <ThemedText style={styles.emptySubtext}>Start a conversation with other users</ThemedText>
          </ThemedView>
        }
      />
          ...
          <View style={styles.modalContent}>
            <View style={styles.inputGroup}>
              ...
              <TextInput
                ...
                onChangeText={(text) => setNewMessage(prev => ({ ...prev, receiver_email: text }))}
