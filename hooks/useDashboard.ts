...
export function useDashboard(farmerId: string) {
      ...
      try {
        ...
        const ordersResponse = await orderService.getAll({
          filters: [
            ...
            { column: 'status', operator: 'in', value: ['pending', 'processing', 'shipping'] }
          ]
        });
        ...
        const paymentsResponse = await paymentService.getAll({
          filters: [
            ...
            { column: 'created_at', operator: 'gte', value: sixtyDaysAgo.toISOString() }
          ]
        });
        ...
        const pendingShipments = ordersResponse.success ? 
          ordersResponse.data?.filter(order => order.status === 'confirmed').length ?? 0 : 0;

        // Format tasks
        const pendingTasks = tasksResponse.success ? tasksResponse.data?.map(task => ({
          id: task.id,
          title: task.title,
        ...
        if (listingsResponse.success && listingsResponse.data) {
          for (const listing of listingsResponse.data) {
            ...
            if (listing.quantity_available <= 20) {
              const productResponse = await productService.getById(listing.product_id);
        ...
        setData({
          ...
          revenueGrowth,
          activeOrders: ordersResponse.success ? ordersResponse.data?.length || 0 : 0,
          pendingShipments,
